...
export const ROUTER_VERSION = "job-router/3.0.0";
export const ROUTER_POLICY_SCHEMA = "job-router-policy/3";
export const ROUTER_FACTS_SCHEMA = "job-router-facts/3";
...
export function validateRouterPolicy(policy) {
  ...
  if (typeof p.version !== "string" || !/^[A-Za-z0-9._-]{1,32}$/.test(p.version)) f.push("POLICY_VERSION");
  if (typeof p.tenant !== "string" || !/^[A-Za-z0-9_-]{1,64}$/.test(p.tenant)) f.push("POLICY_TENANT");
  if (typeof p.costUnit !== "string" || !p.costUnit) f.push("POLICY_COST_UNIT");
  if (!istKarte(p.models) || !Object.keys(p.models).length) f.push("POLICY_MODELS");
    ...
    for (const [key, m] of Object.entries(p.models)) {
      ...
      if (!(Number.isInteger(m.contextTokensMax) && m.contextTokensMax > 0)) f.push("POLICY_MODEL_CONTEXT:" + key);
  ...
  if (!istKarte(p.secondOpinion) || !(Number.isInteger(p.secondOpinion.maxUnits) && p.secondOpinion.maxUnits >= 0)) f.push("POLICY_SECOND_OPINION");
...
export function validateRouterFacts(facts) {
  ...
  if (!istKarte(t)) f.push("FACTS_TASK");
    ...
    if (!(Number.isInteger(t.sourceVersion) && t.sourceVersion >= 1)) f.push("FACTS_SOURCE_VERSION");
    ...
    if (typeof t.goal !== "string" || !t.goal.trim()) f.push("FACTS_GOAL");
    if (!istKarte(t.expectedReturn) || typeof t.expectedReturn.format !== "string" || !Array.isArray(t.acceptanceCriteria)) f.push("FACTS_RETURN_CONTRACT");
  }
  const c = x.context;
  if (!istKarte(c)) f.push("FACTS_CONTEXT");
    ...
    if (c.tokensMeasured !== null && !(Number.isInteger(c.tokensMeasured) && c.tokensMeasured >= 0)) f.push("FACTS_TOKENS_MEASURED");
  }
  const cap = x.capabilities;
  if (!istKarte(cap) || typeof cap.leadershipCanDo !== "boolean") f.push("FACTS_CAPABILITIES");
  const b = x.budget;
  if (!istKarte(b) || !(b.availableUnits === null || istZahl(b.availableUnits)) || typeof b.unit !== "string") f.push("FACTS_BUDGET");
  const m = x.measurements;
  if (!istKarte(m)) f.push("FACTS_MEASUREMENTS");
  else {
    if (m.self !== null && !(istKarte(m.self) && istZahl(m.self.units) && istIso(m.self.measuredAt))) f.push("FACTS_MEASUREMENT_SELF");
    if (!istKarte(m.delegation)) f.push("FACTS_MEASUREMENT_DELEGATION");
    else for (const [ex, d] of Object.entries(m.delegation)) {
      if (!PROVIDERS.includes(ex) || !istKarte(d) || !istZahl(d.executionUnits) || !istZahl(d.handoverUnits) || !istZahl(d.reviewUnits) || !istIso(d.measuredAt)) f.push("FACTS_MEASUREMENT_DELEGATION:" + ex);
    }
  }
  const r = x.risk;
...
function kandidatPruefen({ policy, facts, now }, executor, modelKey, capability, rolle) {
  ...
  if (rolle === "self") {
    ...
    if (!s) ablehnungen.push("MEASUREMENT_MISSING:self");
    else kosten = { execution: s.units, handover: 0, review: 0, total: s.units };
  } else if (rolle === "delegate" || rolle === "second_opinion") {
    const d = facts.measurements.delegation[executor];
    if (!d) ablehnungen.push("MEASUREMENT_MISSING:" + executor);
    else kosten = { execution: d.executionUnits, handover: d.handoverUnits, review: d.reviewUnits, total: d.executionUnits + d.handoverUnits + d.reviewUnits };
  }
  ...
  return { executor, modelKey, provider: m.provider, modelId: m.modelId, role: rolle, cost: kosten, eligible: ablehnungen.length === 0, rejections: ablehnungen };
}
...
function kandidaten(input, executors, capability, rolle) {
  ...
  for (const ex of executors) {
    ...
    if (!Object.values(input.policy.models).some((m) => m.provider === ex)) out.push({ executor: ex, modelKey: null, provider: ex, modelId: null, role: rolle, cost: null, eligible: false, rejections: ["MODEL_NOT_CONFIGURED"] });
  }
  return out;
}
...
export function planJobRoute({ policy, facts, now } = {}) {
  ...
  if (t.taskClass === "deterministic") {
    ...
    if (!facts.risk.authorityConfirmed) {
      // Ungeklaerte Befugnis: Entwurf. Konfidenz, Mehrheiten oder Stimmen
      // sind Daten und keine Freigabe.
      reasons.push({ code: "AUTHORITY_UNCONFIRMED_DRAFT", detail: { confidence: facts.risk.confidence ?? null, votes: facts.risk.votes ?? null } });
...
export function manifestBauen({ policy, facts, route, reasons }) {
  ...
  return {
    ...
    secrets: "none",
    allowedTools: ex ? (facts.task.requiredTools || []).filter((tool) => (policy.tools[ex] || []).includes(tool) && grant.tools.includes(tool)) : [],
