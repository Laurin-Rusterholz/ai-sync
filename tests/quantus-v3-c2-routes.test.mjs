...
test("die Verdrahtung nutzt die echte Idempotenz und lässt fehlende Fachadapter gesperrt", async () => {
  const { buildRuntimeDeps } = await import(path.join(root, "netlify/lib/quantus-v3-runtime.mjs"));
  const idempotency = await import(path.join(root, "netlify/lib/quantus-v3-idempotency.mjs"));
  const deps = await buildRuntimeDeps({ write: true, read: () => undefined });
  // Der Integrationsstand enthält die geprüfte Idempotenz, aber noch keinen
  // Fachadapter. Der vorhandene Port muss genau auf das echte Modul zeigen.
  assert.equal(deps.idempotency.prepare, idempotency.prepareIdempotentCommand);
  assert.equal(deps.idempotency.apply, idempotency.applyIdempotentCommand);
