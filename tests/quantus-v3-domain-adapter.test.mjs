...
test("C3a-01 Fabrik nach C3b-Vertrag (4379061, Blob verifiziert): fuenf Methoden; fehlende Backendkonfiguration ist 503 mit Namen; alle 23 Verben gebunden", async () => {
  for (const f of ["netlify/lib/quantus-v3-runtime.mjs", "netlify/lib/quantus-v3-identity-access.mjs"]) {
    const erwartet = execFileSync("git", ["rev-parse", `4379061:${f}`], { cwd: ROOT, encoding: "utf8" }).trim();
    const ist = execFileSync("git", ["hash-object", f], { cwd: ROOT, encoding: "utf8" }).trim();
    assert.equal(ist, erwartet, `${f} weicht von C3b 4379061 ab`);
  }
  const a = adapter();
