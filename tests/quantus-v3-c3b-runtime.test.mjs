...
test("die Verdrahtung sagt ehrlich, was fehlt", async () => {
  ...
  await imLauf({}, async ({ read }) => {
    ...
    assert.equal(ohneFabrik.wiring.domainReason, "domain_factory_failed");
    ...
    assert.equal(vollstaendig.wiring.identityAccessSource,
      typeof INTEGRATION.firebase.getIdentityAccessToken === "function"
        ? "firebase:getIdentityAccessToken" : "oauth_refresh_exchange");
