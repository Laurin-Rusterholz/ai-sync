...
const source = ["01", "02", "03"].map((part) => read(`public/quantus-bundles/career-core.${part}.txt`)).join("");
vm.runInThisContext(source, { filename: "career-model-core.bundle.js" });
const Core = globalThis.CareerModelCore;
assert.ok(Core, "CareerModelCore must register globally");

const sample = JSON.parse(read("public/career-model-sample.json"));
...
first.progress[sample.module.id].completedDays["day-01"] = { completedAt: "2026-08-04T18:00:00.000Z", actualMinutes: 31 };
...
assert.equal(second.progress[sample.module.id].completedDays["day-01"].actualMinutes, 31);
...
const prompt = Core.buildClaudePrompt({ areaName: "Versicherungen", moduleTitle: "Fahrzeugversicherung", dailyMinutes: 30, notes: "Nutze meine internen Produkteunterlagen." });
