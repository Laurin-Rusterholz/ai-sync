...
| F | Authorized source adapters, mail ledger, document extraction, live UI | First real vertical slice (this checkout): read-only, paginated Gmail source (`runtime/quantus-v3/src/gmail-source.mjs`) feeding `recordSourceCheck` on the real domain kernel; fills the previously-empty `sectionWork` port. Document extraction and other required-source adapters (only Gmail is implemented) remain open; see `quantus-v3-runtime-cloud.md`. |
| G | Job-scoped Claude/Gemini providers, review and untrusted-input isolation | Pure G1 d803f57 integrated with 14 authored + 6 independent tests. This checkout adds a real Sonnet (Anthropic Messages API) HTTP transport (`runtime/quantus-v3/src/anthropic-transport.mjs`) with delimited/labelled untrusted-source isolation, dispatched exclusively through the already-reviewed `cost-adapter.mjs` (reservation, claim, settle/unknown — not reimplemented), filling the previously-empty `costPolicy` port from operator-supplied JSON only. Gemini, human review of provider output, and real backend/IAM attestation remain open. |
