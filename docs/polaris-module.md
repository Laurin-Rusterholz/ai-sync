...
| `polaris/inbox/{type}/{id}` | `{ title?, …, ts, updatedAt, op: 'create'\|'update'\|'delete', … }` — n8n schreibt hier Einträge, die das Frontend **konfliktfrei ins App-Modell merged** (ein einziger `on('value')`-Listener über die ganze Inbox → deckt auch künftige Typen ab; REST-Poll-Fallback). **`type`-Namen sind 1:1 mit der n8n prep-entity-Whitelist** (kanonisch, siehe Tabelle unten). Jeder `type` → Ziel: `entities.<kind>` (via `createEntity`/`updateEntity`), Direktschreibung (`person/concept/thesis`) oder **Array-Modul** (`habit→dailyBriefing.routines`, `reflection→reflections`, `journal→journal.documents`, `belief→dailyBriefing.beliefs`, `timeCapsule→journal.selfLetters`, `rule→rules.books`: create=push, update=patch by id, delete=**remove** by id). Kanonische Typen ohne eigenes App-Schema → generisch `entities.polaris_<type>` (kein Verlust). Unbekannte/nicht-kanonische Typen → ignoriert + geloggt. **Verbindliche Sicherheitsgrenze:** `POLARIS_BLOCKED_TYPES` lehnt Passwort-Tresor (`password/passwords/passwordmanager/vault/tresor`) und Bank-/Kontodaten (`account/konto/transaction/creditcard/subscription/invoice/financialgoal/purchaseproposal/budget/iban`) **immer** ab. Regeln: **nie** Direktschreiben in `appStore/app-data_json`; LWW nur wenn Polaris' `updatedAt` neuer als `polarisTs`; Map-`op:'delete'` → Soft-Delete (`status:'deleted'`), Array-`op:'delete'` → Element entfernen; **BLOCK-Feldfilter** `POLARIS_SENSITIVE_RE` (`password/passwort/pass/pin/iban/bic/apikey/api_key/secret/token/cardnumber/card_number/cvv`, rekursiv, case-insensitive) auf **alle** Typen inkl. Array-Module. Nach Merge `processedAt`+`processedBy` → kein Doppel-Merge. Aktivitäts-Pille unten links. |

### Kanonische entity-Typen (Frontend ↔ n8n, identisch)

`note, goal, idea, decision, event, habit, task, project, organization, person, concept, meeting, reflection, journal, flashcard, strategy, program, thesis, sticky, measure, vocabulary, document, dailyBriefing, weekPlanning, workload, message, timeCapsule, emailTemplate, chapter, timeLog, translation, deck, milestone, risk, resource, stakeholder, workflowStep, template, statusUpdate, retro, reflectionQuestion, belief, focusItem, knowledgeArea, learningSession, question, ideaCard, swot` (+ `rule` — nicht kanonisch, aber fürs `rules.books`-Array im Frontend bedienbar; falls n8n `rule` senden soll, dort ergänzen).

**Mapping (Frontend-Ziel):** `event→entities.calendarEvents`, `message→entities.scheduledMessages`, `timeLog→entities.timeEntries`, `vocabulary→entities.dictionary`, `person/concept/thesis→entities.persons/concepts/theses` (direkt); `habit→dailyBriefing.routines`, `reflection→reflections`, `journal→journal.documents`, `belief→dailyBriefing.beliefs`, `timeCapsule→journal.selfLetters`, `rule→rules.books` (Arrays); `note/goal/idea/decision/task/project/organization/meeting/strategy/program/resource/deck→entities.<kind>`; alle übrigen kanonischen Typen → `entities.polaris_<type>`.

**Angleich alt→neu (aus PR #139):** `calendarEvent→event`, `scheduledMessage→message`, `timeEntry→timeLog` (+ deutsche Aliase `projekt/aufgabe/termin/…` entfernt zugunsten der kanonischen Namen). `notebook/protocol/workflow/contact/article/update` waren nicht kanonisch → durch kanonische Äquivalente bzw. generischen Fallback ersetzt.
...
6. **n8n entity-Handler (separat, nicht im Repo):** Die Typ-Whitelist des
   generischen entity-create/update/delete/list-Handlers muss dieselben
   `type`-Namen verwenden wie das Frontend (`POLARIS_INBOX_TYPES`, Liste in der
   Datenmodell-Tabelle oben) und jeden Eintrag nach `polaris/inbox/{type}/{id}`
   mit `{op, title, …, ts, updatedAt}` schreiben. **Verbindlich:** Tresor-
   (`password*`, `vault`, `tresor`) und Bank-/Kontotypen (`account`, `konto`,
   `transaction`, `creditcard`, `subscription`, `invoice`, `budget`, `iban`)
   **nicht** aufnehmen; den BLOCK-Feldfilter (`password/passwort/pass/pin/iban/
   bic/apikey/api_key/secret/token/cardnumber/card_number/cvv`) serverseitig auf
   **alle** Typen anwenden. Das Frontend erzwingt beide Grenzen zusätzlich
   (`POLARIS_BLOCKED_TYPES` + `POLARIS_SENSITIVE_RE`), aber der Handler sollte
   sie bereits vor dem RTDB-Schreiben durchsetzen. `list` ist ein reiner
   Server-Read (liest `entities.*` bzw. `appStore/app-data_json`, siehe
   `docs/polaris-budget-access.md`).
