...
| `polaris/inbox/{type}/{id}` | `{ title, description?, status?, source?, ts, updatedAt, op: 'create'\|'update'\|'delete', … }` — n8n schreibt hier neue/aktualisierte Einträge, die das Frontend **konfliktfrei ins App-Modell merged**. **Whitelist `type` (alle unbedenklichen Module + dt. Aliase):** `project/projekt, task/aufgabe, note/notiz, goal/ziel, idea/idee, meeting/besprechung, decision/entscheidung, organization/organisation, program/programm, strategy/strategie, article/artikel, update, calendarEvent/termin/kalender, scheduledMessage/nachricht/message, notebook/notizbuch/notiz-kapitel, concept/konzept, thesis/these, protocol/protokoll, workflow/arbeitsablauf, person/kontakt, contact, resource, timeEntry`. Jeder `type` → `entities.<kind>` (via `createEntity`/`updateEntity` + `scheduleSave`; für `person/concept/thesis/protocol/workflow` — nicht in `getEntityMap` — Direktschreibung in `entities.<key>`). Unbekannte types → ignoriert + geloggt. **Verbindliche Sicherheitsgrenze:** `POLARIS_BLOCKED_TYPES` lehnt Passwort-Tresor (`password/passwords/passwordmanager/vault/tresor`) und Bank-/Kontodaten (`account/konto/transaction/creditcard/subscription/invoice/financialgoal/purchaseproposal/budget/iban`) **immer** ab. Regeln: **nie** Direktschreiben in `appStore/app-data_json`; LWW nur wenn Polaris' `updatedAt` neuer als `polarisTs`; `op:'delete'` → Soft-Delete (`status:'deleted'`); **BLOCK-Feldfilter** `POLARIS_SENSITIVE_RE` = `password/passwort/pass/pin/iban/bic/apikey/api_key/secret/token/cardnumber/card_number/cvv` (rekursiv, case-insensitive) auf **alle** Typen — weder Modell noch Anzeige. Nach Merge `processedAt`+`processedBy` (kein Löschen) → kein Doppel-Merge. Aktivitäts-Pille unten links. |
